...
bt_node* zs_interpret_expression_input_core(char* input_expr, size_t lineno, size_t* counter, bt_node* tree);
...
size_t create_parentesis(bt_node** currentNode, bt_node** tree, zs_expr_parentesis* parentesis, zs_expr_node* temp, zs_expr_type type, zs_bool* error, char* input_where)
{
    ...
    *error = ZS_FALSE;
    ...
    tempnode = zs_interpret_expression_input_core(input_where, 0, &i, where);
    if(tempnode == NULL)
    {
        *error = ZS_TRUE;
    }
...
{
    return zs_interpret_expression_input_core(input_expr, lineno, NULL, NULL);
}

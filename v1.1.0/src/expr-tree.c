...
void print_expr_node(bt_node_data* node_data)
{
    ...
    else if(expr_node->expr_type == ZS_EXPR_TYPE_VECTOR_INITIALIZER)
    {
        printf("{}\n");
    }
    else if(expr_node->expr_type == ZS_EXPR_TYPE_FUNCTION_ENCLOSER_LIST)
    {
        printf("(ARGS)\n");
    }
    else if(expr_node->expr_type == ZS_EXPR_TYPE_VECTOR_INITIALIZER_NEXT_ELEMENT)
    {
        printf(",\n");
    }
    else
...
void read_vector(zs_state* state, bt_node* current, zs_vector* vector)
{
    zs_data tempdata;

    while(1)
    {
        if(TO_NODE_EXPR(current->left)->data.isref)
        {
            tempdata = TO_NODE_EXPR(current->left)->data;
        }
        else
        {
            tempdata = copy_zs_data(&TO_NODE_EXPR(current->left)->data);
        }
        zs_add_element(vector, tempdata, state);
...
int zs_operate_tree(zs_state* state, bt_node* tree)
{
    ...
    else if(TO_NODE_EXPR(tree)->expr_type == ZS_EXPR_TYPE_VECTOR_INITIALIZER || TO_NODE_EXPR(tree)->expr_type == ZS_EXPR_TYPE_FUNCTION_ENCLOSER_LIST)
    {
        zs_create_alloc_data(&data, ZS_TYPE_VECTOR);
        
        data.isref = ZS_FALSE;
        current = tree;
        
        read_vector(state, current, data.data);
    ...
    else if(TO_NODE_EXPR(tree)->expr_type == ZS_EXPR_TYPE_FUNCTION_CALL)
    {
        ...
        vecptr = (zs_vector*)(TO_NODE_EXPR(tree->right)->data.data);
        if(index == -1)
        {
            ret = zs_builin_function_execute(state, (char*)TO_NODE_EXPR(tree->left)->data.data, &data, vecptr->data, vecptr->len);
            if(ret == -1)
            {
        ...
        /* calling function */
        if(zs_call_function(state, index, vecptr->data, vecptr->len) == 0)
        {
